...
% Las cadenas canonicas del paper viven UNA sola vez, aqui: de estas salen tanto la
% portada como los metadatos del documento PDF, de modo que no pueden divergir. El
% verificador del paquete las lee de aqui y comprueba que el PDF compilado las lleva.
\newcommand{\papertitle}{Statistical Structure of the Historical Orderings of the I Ching Hexagrams}
\newcommand{\papersubtitle}{Pair Rule, Family Gradient, and the Limits of Demonstrability}
\newcommand{\paperauthor}{Alexis García Hurtado}

% Metadatos del documento. unicode=true para que el apellido con acento se guarde como
% texto Unicode y no como un byte de una codificacion heredada.
\hypersetup{
  unicode=true,
  pdftitle={\papertitle: \papersubtitle},
  pdfauthor={\paperauthor},
  pdfsubject={Statistical analysis of the King Wen, Mawangdui and Jing Fang orderings of the I Ching hexagrams},
  pdfkeywords={I Ching; King Wen sequence; hexagrams; combinatorics; Monte Carlo; conditional null models; history of mathematics},
}

\title{\papertitle\\[0.35em]
  {\large \papersubtitle}}

\author{\paperauthor\thanks{Independent Researcher, Gatineau, Québec, Canada. Email: research@theoriginaliching.com. ORCID: 0009-0003-4636-8206.}}
\date{July 25, 2026}
...
\bibitem{garciahurtado2026} García Hurtado, A. (2026). Uninformative rungs: an order-theoretic stopping criterion for nested reference sets [Preprint]. Zenodo. \url{https://doi.org/10.5281/zenodo.21750029}
...
Section~\ref{sec:chan} recorded the standard caveat of conditional inference: results are relative to the chosen null. This appendix probes that caveat directly. We re-evaluate the four signatures of Table~\ref{tab:chan} under a ladder of six nulls ordered by containment, an order that is partial rather than a chain: each rung concedes some part of the sequence's architecture, not uniformly more than the rung before it, and we ask what remains: (P0) the free shuffle; (P1) the pair-preserving null of Section~\ref{sec:chan}; (P2) pairs permuted only within the two received canons (15 and 17 pairs), the received division of the text into hexagrams 1-30 and 31-64; (P3) the pair null with the four palindrome pairs, hexagrams (1,2), (27,28), (29,30) and (61,62), fixed in position and orientation (these are the pairs whose members are invariant under rotation and are therefore matched by complementation; detecting complement pairs by value, \(a\,\mathrm{XOR}\,b = 63\), would instead catch eight pairs, since in four rotation pairs reversal and complementation coincide); (P4) the sixteen blocks of four consecutive positions permuted among themselves, so that a pair never leaves its block, with the two pairs inside a block free to exchange and all orientations free; and (P5) the received pair order with only the 32 orientations randomized. An order-theoretic reading of this family, and of when a further rung stops being informative, is given in a companion preprint (García Hurtado, 2026). All rungs are Monte Carlo with 20,000 samples and seed 20260722; percentiles are $100\,P(\text{statistic} < \text{observed})$.
